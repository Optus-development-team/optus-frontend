...
### Fiat Payload (BMS-specific)

For QR payments the backend also accepts a lightweight fiat payload:
...
Only one payment method (crypto or fiat) is processed per order. A failed fiat verification will return HTTP 402 with refreshed payment options.
