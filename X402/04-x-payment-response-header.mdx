...
- `type` | string | Yes | `crypto` or `fiat` |
- `transaction` | string | Yes (if success) | Transaction hash on blockchain or fiat reference |
- `network` | string | Yes (crypto) | Network where transaction was settled |
- `payer` | string | Yes (crypto) | Address of the payer |
- `currency` | string | Yes (fiat) | Fiat currency code |
