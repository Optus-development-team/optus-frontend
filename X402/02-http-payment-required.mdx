...
> BMS backend nuance: the `accepts` array can include a fiat QR option with `{ "type": "fiat", "AmountRequired": "1500", "Base64QRSimple": "...", "currency": "BOB", "symbol": "Bs.", "maxTimeoutSeconds": 60 }` when the QR is generated within 30 seconds; otherwise only the crypto option is returned.
