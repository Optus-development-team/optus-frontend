...
> Implementation note (BMS backend): `GET /api/pay` now serves as the single entrypoint for both crypto and fiat QR payments. When the request is made without `X-PAYMENT`, the service returns HTTP 402 with crypto requirements and will only include a fiat QR option if Playwright can generate the QR within 30 seconds. Only one payment method (crypto **or** fiat) is honored per order.
